...
static int mmc_clk_io_onoff(int sdc_no, int onoff)
{
	...
	*(volatile unsigned int *)(0x06000800 + 0x314) = 0xA; //BIAS
		...
		boot_set_gpio((void *)BT0_head.prvt_head.storage_gpio, 10, 1);
	}
	else // if(sdc_no == 2)
	{
		boot_set_gpio((void *)(BT0_head.prvt_head.storage_gpio + 16), 10, 1);
	}
...
#else
	dumphex32("ccmu", (char*)CCMU_REGS_BASE, 0x100);
#endif
